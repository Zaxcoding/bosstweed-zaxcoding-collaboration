...
public class LevelEditor
{
	...
	boolean drawGrid = true, lowRes = EDITOR_RESOLUTION_X < 1280, settingPartner, settingStart;
	boolean mouseLockX = false, mouseLockY = false;
	
	private String currShape = "Box", inputValue, skyHex = "default", fileName = "Unnamed";
	...
	private void mouseInput()
	{
		...
		if (Mouse.isButtonDown(0) && mouseIn(0, 50, TOP + 8*FONT_SIZE, TOP + 9*FONT_SIZE))
			buttonCode = 1;			// i
		if (Mouse.isButtonDown(0) && mouseIn(75, 175, TOP + 8*FONT_SIZE, TOP + 9*FONT_SIZE))
			buttonCode = 2;			// type
		if (Mouse.isButtonDown(0) && mouseIn(0, 200, TOP + 9*FONT_SIZE, TOP + 10*FONT_SIZE))
		{
		...
		if (Mouse.isButtonDown(0) && mouseIn(0, LEFT - 70, TOP + 10*FONT_SIZE + 5, TOP + 11*FONT_SIZE + 5))
		{
			current.moving = !current.moving;
			fixMouse();
		}
		
		if (Mouse.isButtonDown(0) && mouseIn(0, 125, TOP + 11*FONT_SIZE + 5, TOP + 12*FONT_SIZE + 5))
		{
			current.vert = !current.vert;
			fixMouse();
		}
		if (Mouse.isButtonDown(0) && mouseIn(135, 245, TOP + 11*FONT_SIZE + 5, TOP + 12*FONT_SIZE + 5))
		{
			current.on = !current.on;
			fixMouse();
		}
		if (Mouse.isButtonDown(0) && mouseIn(0, LEFT - 20, TOP + 14*FONT_SIZE + 5, TOP + 15*FONT_SIZE + 5))
		{
			setSky();
			fixMouse();
		}
		if (Mouse.isButtonDown(0) && mouseIn(0, LEFT - 50, TOP + 16*FONT_SIZE + 5, TOP + 17*FONT_SIZE + 5))
		{
	...
	private void movementStuff()
	{
		...
		if (inputValue != null && !inputValue.equals(""))
		{
			...
			{
				inputValue = JOptionPane.showInputDialog("Current bottom of the piece: " + (selected.partner.y + selected.partner.height- FUDGE_Y) + ", enter end Y.");
					if (inputValue != null && !inputValue.equals(""))
					{
						selected.partner.endPos = Double.parseDouble(inputValue) + FUDGE_Y - selected.partner.height;
						selected.partner.startPos = selected.partner.y;
					}
			}
			if (!selected.partner.upDown)
			{
				inputValue = JOptionPane.showInputDialog("Current right side of the piece: " + (selected.partner.x + selected.partner.width - FUDGE_X) + ", enter end X.");
				if (inputValue != null && !inputValue.equals(""))
				{
				selected.partner.endPos = Double.parseDouble(inputValue) + FUDGE_X - selected.partner.width;
	...
	private void drawText()
	{
		...
		uniFont.drawString(5, TOP + 8*FONT_SIZE, iString + current.i + "     " + typeString + current.type);
		uniFont.drawString(5, TOP + 9*FONT_SIZE, "moveSpeed: " + current.moveSpeed);
		uniFont.drawString(5, TOP + 10*FONT_SIZE, "moving: " + current.moving);
		
		uniFont.drawString(5, TOP + 11*FONT_SIZE, "vert: " + current.vert + "   on: " + current.on);
		...
		uniFont.drawString(5, TOP + 16*FONT_SIZE, "Start x,y: " + (startX - FUDGE_X) + "," + (startY - FUDGE_Y));
		...
		if (selected != null)
		{
			...
			uniFont.drawString(RIGHT + 5, TOP + FONT_SIZE, "x,y pos: " + (selected.x - FUDGE_X) + "," + (selected.y - FUDGE_Y));
	...
	{
		fileName = JOptionPane.showInputDialog("Enter the filename to load please: ");
	...
	{
		fileName = JOptionPane.showInputDialog("Enter the desired filename please: ");
		if (fileName != null && !fileName.equals(""))
		{
			try
			{
